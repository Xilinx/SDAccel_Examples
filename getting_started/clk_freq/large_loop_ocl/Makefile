...
# Building kernel
$(XCLBIN)/cnn_GOOD.$(TARGET).$(DSA).xo: ./src/cnn_convolution.cl
	mkdir -p $(XCLBIN)
	$(XOCC) $(CLFLAGS) -c -k cnn -I'$(<D)' -o'$@' '$<'
$(XCLBIN)/cnn_BAD.$(TARGET).$(DSA).xo: ./src/cnn_convolution_bad.cl
	mkdir -p $(XCLBIN)
	$(XOCC) $(CLFLAGS) -c -k cnn -I'$(<D)' -o'$@' '$<'

$(XCLBIN)/cnn_GOOD.$(TARGET).$(DSA).xclbin: $(BINARY_CONTAINER_cnn_GOOD_OBJS)
	$(XOCC) $(CLFLAGS) -l $(LDCLFLAGS) --nk cnn:4 -o'$@' $(+)
$(XCLBIN)/cnn_BAD.$(TARGET).$(DSA).xclbin: $(BINARY_CONTAINER_cnn_BAD_OBJS)
	$(XOCC) $(CLFLAGS) -l $(LDCLFLAGS) --nk cnn:1 -o'$@' $(+)
