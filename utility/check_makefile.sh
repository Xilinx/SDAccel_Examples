...
echo "ignoring: getting_started/clk_freq/"
...
for f in $VCS_FILES; do
	if [[ ($f == */description.json) && ($f != */kernel_global_bandwidth/*) && ($f != */kmeans/*) && ($f != */clk_freq/*) && ($f != */sha1/*) && ($f != */rtl_kernel/*) && ($f != */high_perf_mat_mult/*) ]]; then
